...
EXEMPT = re.compile(r"/(resources|target)/")
...
if denied:
    reason = (
        "Blocked: text read of indexed Scala source (" + subject + "). Read these "
        ...
        "Fixtures under resources/ and any source the build has not emitted SemanticDB "
        "for carry no index entry and stay readable here."
    )
    print(json.dumps({
        "hookSpecificOutput": {
